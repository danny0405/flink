...
public class WindowJoinOperatorTest<K, W extends Window> {
	...
	private InternalTypeInfo<RowData> inputType = InternalTypeInfo.ofFields(
			new IntType(),
			new VarCharType(VarCharType.MAX_LENGTH),
			new BigIntType());

	// Key selector for both inputs.
	private BinaryRowDataKeySelector keySelector = new BinaryRowDataKeySelector(new int[] { 0 }, inputType.toRowFieldTypes());
	private TypeInformation<RowData> keyType = keySelector.getProducedType();

	private JoinInputSideSpec inputSideSpec1 = JoinInputSideSpec.withUniqueKeyContainedByJoinKey(
			InternalTypeInfo.ofFields(new IntType()),
			keySelector);
	private JoinInputSideSpec inputSideSpec2 = JoinInputSideSpec.withUniqueKey(
	...
	private TwoInputStreamOperatorTestHarness<RowData, RowData, RowData> createTestHarness(
			WindowJoinOperator<K, W> operator)
			throws Exception {
