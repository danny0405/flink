 ...
 *
 * <p>Copied from Calcite class, the modifications:
 * - line~442, window table function timeCols are always needed by the Join.
 */
public class PushProjector {
	...
	/**
	 * The window table function timeCols are always needed by the join.
	 */
	private boolean hasWindowFunctionCall() {
		if (childRel instanceof Join) {
			RelNode left = ((Join) childRel).getLeft();
			RelNode right = ((Join) childRel).getRight();
			return WindowJoinUtils.hasWindowFunctionScan(left)
