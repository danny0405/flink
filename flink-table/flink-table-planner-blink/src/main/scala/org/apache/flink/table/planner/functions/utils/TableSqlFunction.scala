  ...
  private[flink] def convertArguments(
    ...
    0 until callBinding.getOperandCount foreach { i =>
      ...
      val value: Object = if (callBinding.isOperandLiteral(i, true)) {
        callBinding.getOperandLiteralValue(i, operandType)
      } else {
        if (operandType.isNullable) {
          null
        } else {
          throw new IllegalArgumentException(s"All arguments of call to function $opName" +
            s" should be literal. Actual argument #$i is not literal")
        }
      }
      arguments.add(value);
    }
